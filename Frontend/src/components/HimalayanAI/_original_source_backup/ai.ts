...
import { VALUE_PRODUCTS, calcProfit, calcMargin, WEATHER_DATA } from './valueadd';
import {
  CROP_CALENDAR, ROOM_PRICING, MENU_SUGGESTIONS,
  ORGANIC_VS_MARKET, CUSTOMER_STATS, STAY_LISTINGS,
  MARKET_COMPARE, ATTRACTION_TIPS, SEASONAL_FOOD,
} from './roledata';

export type UserRole = 'farmer' | 'homestay' | 'user';

export type Lang = 'en' | 'hi' | 'hing';
...
const HINGLISH_WORDS =
  /\b(kya|hai|hain|kaise|banate|banau|banaye|batao|bata|bhaiya|bhai|mujhe|mera|meri|kitna|kitne|bhav|daam|kheti|fasal|karo|chahiye|dikhao|acha|accha|theek|nahi|nhi|haan|poochho|karein|krna|karna|wala|wali|ka|ki|ke|se|mein|aur|recipe batao|kaha|kahan|milega|lena|kharidna|order)\b/i;

export const detectLang = (q: string): Lang => {
  if (/[\u0900-\u097F]/.test(q)) return 'hi';
  if (HINGLISH_WORDS.test(q)) return 'hing';
  return 'en';
};
...
export const findRecipe = (query: string): { key: string; recipe: Recipe } | null => {
  const q = normalize(query);
  const qTokens = q.split(' ').filter((t) => t.length > 1 && !STOP_WORDS.has(t));
  if (qTokens.length === 0 && q.length === 0) return null;
  ...
  for (const [key, recipe] of Object.entries(RECIPES)) {
    ...
    for (const alias of recipe.aliases) {
      ...
      if (q.includes(a)) score = Math.max(score, 100 + a.length);
    }
    ...
    for (const token of qTokens) {
      if (recipeTokens.has(token)) overlap += 10;
      else {
  ...
  if (bestKey && bestScore >= 10) return { key: bestKey, recipe: RECIPES[bestKey] };
  return null;
};

// ---------- Response builders ----------
const buildRecipeText = (recipe: Recipe, lang: Lang): string => {
  const t = T[lang];
  return (
...
const matchProducts = (q: string): Product[] => {
...
// ---------- Main AI ----------
export const generateAIResponse = (userMessage: string, role: UserRole = 'user'): AIResponse => {
  ...
  if (
    ...
    const seasonKey =
      month >= 1 && month <= 3 ? 'Spring (Feb-Apr)'
    ...
    return {
      text:
        `🌱 **AI Crop Advisor** (Professional Recommendation)\n\n` +
        `📅 **Grow NOW — ${seasonKey}:**\n${currentRows}\n\n` +
  ...
  if (
    ...
    if (item) {
      ...
      return {
        text:
          `📊 **Market Price Comparison — ${item.product}** ${item.emoji}\n\n${rows}\n\n` +
    ...
    return {
      text:
        `📊 **Market Price Comparison (Live from major sites)**\n\n${all}\n\n` +
  ...
  if (
    ...
    return {
      text:
        `⚖️ **AI Grow & Sell Planner (Stay in Profit)**\n\n` +
  ...
  if (
    ...
    return {
      text:
        `🎯 **AI Marketing Coach — How to Attract Customers**\n\n${ATTRACTION_TIPS.map((tip) => tip).join('\n\n')}\n\n` +
  ...
  if (
    ...
    return {
      text:
        `📊 **AI Market Analysis — What to Sell & At What Price**\n\n` +
  ...
  if (
    ...
    return {
      text:
        `🏡 **AI Room Pricing Advisor — ${loc.location}** ${loc.emoji}\n\n` +
  ...
  if (
    ...
    let key: keyof typeof SEASONAL_FOOD;
    if (/(sardi|thand|winter|cold|ठंड|सर्दी)/.test(q)) key = 'winter';
    else if (/(garmi|summer|hot|गर्मी)/.test(q)) key = 'summer';
    else if (/(barsaat|barish|monsoon|बरसात)/.test(q)) key = 'monsoon';
    else {
  ...
  if (
    ...
    return {
      text:
        `🍽️ **AI Organic Menu Planner — Keep Guests Happy!**\n\n` +
  ...
  if (
    /(organic vs|vs market|market product|difference.*organic|organic.*difference|market se|market wale|fark|फर्क|अंतर|kyu organic|why organic|benefit.*organic|organic.*benefit)/.test(q)
  ) {
    const rows = ORGANIC_VS_MARKET.map(
      (o) => `${o.emoji} **${o.aspect}**\n   ✅ Organic: ${o.organic}\n   ❌ Market: ${o.market}`
    ).join('\n\n');
    const statLine =
      role === 'homestay'
    ...
    return {
      text:
        `🌿 **Organic vs Market Products — The Honest Comparison**\n\n${rows}${statLine}\n\n` +
  ...
  if (
    ...
    if (locMatch) {
      ...
      return {
        text:
          `🏡 **Best Stay in ${locMatch.location}** ${locMatch.emoji}\n\n` +
    ...
    const list = STAY_LISTINGS.map(
      (s) =>
        `${s.emoji} **${s.name}** (${s.location}) — ${s.rating}★\n   ₹${s.price.toLocaleString('en-IN')}/night (OTA avg ₹${s.otaAvg.toLocaleString('en-IN')} — save ₹${(s.otaAvg - s.price).toLocaleString('en-IN')}!)\n   🎯 ${s.bestFor}`
    ).join('\n\n');
    return {
      text:
        `🏔️ **AI Stay Recommendations — Compared with OTA Market Rates**\n\n${list}\n\n` +
  ...
  if (found && (isRecipeIntent || found.recipe.aliases.some((a) => q.includes(normalize(a))))) {
    if (role === 'farmer') {
      ...
      return {
        text:
          `💼 **Business Intel — ${found.recipe.name}** ${found.recipe.emoji}\n\n` +
  ...
  if (
    ...
    return {
      text:
        `🏡 **AI Homestay Design & Hygiene Coach**\n\n` +
  ...
  if (
    ...
    return {
      text:
        `🏔️ **Why Stay at a Himalayan Connect Homestay?**\n\n` +
  ...
  if (/(weather|mausam|baarish|rain|temperature|forecast|मौसम|बारिश)/.test(q)) {
    const district =
      WEATHER_DATA.find((w) => q.includes(w.district.toLowerCase())) ?? WEATHER_DATA[0];
    const fc = district.forecast.map((f) => `${f.day}: ${f.icon} ${f.temp} (rain ${f.rain}%)`).join('\n');
    return {
      text:
        `🌤️ **Weather — ${district.district}** (${district.altitude})\n\n` +
  ...
  if (/(value|processing|brownie|browne|bread|bun|doughnut|donut|laddu|convert|banakar|bech|profit|munafa|shelf life|मुनाफा|लड्डू)/.test(q)) {
    ...
    return {
      text:
        `🏭 **AI Value-Addition Suggestions (Top Picks)**\n\n${rows}\n\n` +
  ...
  if (/(burans|buransh|rhododendron|बुरांस|nursery|sapling)/.test(q)) {
    ...
    return {
      text:
        `🌺 **Burans (Rhododendron) — Complete Business Guide**\n\n` +
  ...
  if (/(lingda|lingra|fiddlehead|लिंगड़ा)/.test(q)) {
    ...
    return {
      text:
        `🌿 **Lingda (Fiddlehead Fern) — RICH IN IRON!**\n\n` +
  ...
  // 9. Fallback
  return { text: t.fallback + t.helpline };
};
