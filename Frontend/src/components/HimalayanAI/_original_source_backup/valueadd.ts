...
export interface ValueProduct {
  ...
  emoji: string;
  baseCrop: string;
  rawMaterial: string;      // quantity of raw material needed
  rawCost: number;          // cost of raw material (₹)
  conversionCost: number;   // processing cost (₹)
  outputYield: string;      // what you get
  shelfLife: string;
  marketPrice: number;      // expected selling price (₹)
  unit: string;
  demandNote: string;
  special?: boolean;
}
...
export const VALUE_PRODUCTS: ValueProduct[] = [
  {
...
export const calcProfit = (v: ValueProduct) => v.marketPrice - v.rawCost - v.conversionCost;
export const calcMargin = (v: ValueProduct) =>
  Math.round((calcProfit(v) / (v.rawCost + v.conversionCost)) * 100);

// ---------- WEATHER (geo-tech based, district-wise) ----------
...
export interface DistrictWeather {
  district: string;
  altitude: string;
  now: { temp: number; feels: number; humidity: number; wind: number; icon: string; condition: string };
  forecast: DayForecast[];
  advisory: string;
}

export const WEATHER_DATA: DistrictWeather[] = [
  {
...
// ---------- MARKET NEEDS (AI demand signals) ----------
...
export const MARKET_NEEDS: MarketNeed[] = [
...
export const TRACEABILITY: Record<string, TraceInfo> = {
...
export const getTrace = (productId: string, region: string): TraceInfo =>
...
export const getB2BPricing = (priceValue: number) => {
